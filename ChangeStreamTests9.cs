...
namespace MongoDBTests
{
    ...
    public class AmpsConfigChangeStreamTests
    {
        ...
        public async Task WatchChangesAsync_CallsWatchAsyncOnCollection()
        {
            ...
            mockCursor.Setup(c => c.MoveNextAsync(It.IsAny<CancellationToken>())).ReturnsAsync(false); // End immediately
        ...
        public async Task WatchChangesAsync_HandlesInsertOperation()
        {
            ...
            var cts = new CancellationTokenSource(TimeSpan.FromSeconds(5));
            var mockCursor = new Mock<IAsyncCursor<ChangeStreamDocument<BsonDocument>>>();
            var changeStreamDocs = new List<ChangeStreamDocument<BsonDocument>>
            {
            ...
            // Setup cursor to return one change then complete
            bool firstCall = true;
            ...
            // Assert
            mockCursor.Verify(c => c.MoveNextAsync(It.IsAny<CancellationToken>()), Times.AtLeastOnce());
